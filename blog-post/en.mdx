...
**Superpowers vs Agent Skills vs Matt Pocock's Skills** is not a question of which package is universally better. The right choice depends on the shape of the work: Superpowers favors autonomy with deeper reasoning, Agent Skills covers the full delivery lifecycle, and Matt Pocock's Skills works as a small, opinionated daily toolkit for real engineering.
...
To compare all three with the same task, I created a controlled exercise using the skill files as the operating style. The task was to build a React component called `TaskRadar`, with a PRD, SPEC, and implementation.

The component had to:
...
The artifacts are in `.context/skill-framework-benchmark/`:
...
This was not a timed race with three independent agents. It was a tabletop benchmark: the goal was to compare the kind of output each system encourages when it receives the same task.
